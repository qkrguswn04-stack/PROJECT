...
Output : lightgbm_best.json          (best params + CV score + history)

Usage  : python tune_lightgbm.py
...
OUT_FILE  = Path("lightgbm_best.json")
...
def main():
    print("=" * 60)
    print(f"LightGBM — Optuna Tuning ({N_TRIALS} trials, {N_SPLITS}-fold CV)")
    print("=" * 60)
    ...
    for tr, va in splits:
        ...
        m = lgb.LGBMClassifier(**full_params); m.fit(X_tr, y_tr)
