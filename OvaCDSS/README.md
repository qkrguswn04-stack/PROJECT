...
### 제약 속에서의 학습
외부 인터넷을 사용할 수 없는 AI-HUB 폐쇄망 환경에서 개발하면서 새로운 도전이 있었어. GPU 메모리 에러나 버전 충돌 같은 문제를 에러 메시지만으로 분석하고 논리적으로 해결해야 했어. 이 과정이 코드에 대한 이해를 훨씬 깊게 만들었어.

### 코드의 재현성 중요성
바깥에서 테스트한 코드를 폐쇄망으로 가져갔을 때, 단순히 경로만 수정하면 된다고 생각했는데 틀렸어. 절대경로, PyTorch 버전, 배치 사이즈까지 모두 조정해야 했거든. **경로, 버전, 의존성을 명시적으로 처리하는 게 얼마나 중요한지** 실감했어.

### 문제 해결의 자산화
폐쇄망에서 겪은 모든 문제와 해결 과정을 노션에 기록했어. "왜 SAMUS는 못 썼나", "GPU 메모리 에러 해결법" 같은 기록들이 팀원들이 같은 실수를 반복하지 않도록 하는 자산이 되었어.

---

## 💡 프로젝트를 마치며

이 프로젝트에서 배운 가장 중요한 것은 **데이터 규모보다 도메인 지식이 담긴 가중치의 가치**였어. MMOTU 1,639장으로는 22.9%의 Recall에 불과했지만, 초음파 도메인 지식이 담긴 AI-HUB 가중치를 활용하니 47.2%로 향상되었기 때문이야. 또한 폐쇄망이라는 제약 환경을 경험하면서 코드의 재현성과 의존성 관리의 중요성을 깊이 있게 이해할 수 있었어. 무엇보다 노션을 통해 팀원들과 문제 해결 과정을 공유하면서, 각자가 마주한 시행착오가 팀 전체의 학습 자산이 될 수 있다는 걸 느꼈어. PM으로서 기한 내에 목표를 달성하고 팀원들과 함께 성장하는 경험이 이 프로젝트의 가장 큰 수확이었어.
