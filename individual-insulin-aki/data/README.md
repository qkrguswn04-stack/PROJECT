...
본 프로젝트에서 사용한 데이터는 미국 MIT의 **MIMIC-IV(Medical Information Mart for Intensive Care v2.2)** 비식별화 중환자 데이터베이스입니다. PhysioNet의 데이터 보호 정책에 따라 실제 데이터셋(CSV)은 저장소에 업로드하지 않으며, 동일한 환경을 재현할 수 있는 **추출 쿼리 및 데이터 명세**를 공유합니다.
...
| `insulin_dosage` | Float | 해당 시점에 투여된 인슐린 누적 용량 (Units) (처치 변수) |
